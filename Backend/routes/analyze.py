...
async def analyze_counterfactual(req: CounterfactualRequest):
    ...
    For other patients, makes live Groq API calls.
    """
    # Find the patient
    patient = None
    for p in PATIENTS:
        if p["patient_id"] == req.patient_id:
            patient = p
            break
    ...
    # Check for pre-computed result (demo speed)
    if req.patient_id in COUNTERFACTUAL_PRECOMPUTED:
        precomputed = COUNTERFACTUAL_PRECOMPUTED[req.patient_id].get(model_id)
        if precomputed:
            return {
...
async def analyze_reason(req: ReasonerRequest):
    ...
    if not req.disparity_finding or "33-point" in req.disparity_finding or "P-0142" in req.patient_context:
        return REASONER_PRECOMPUTED.get(model_id, REASONER_PRECOMPUTED["biased"])

    # Live Groq call
    user_msg = f"Disparity finding: {req.disparity_finding}"
    ...
    try:
        ...
        tone = "danger" if "HARMFUL" in classification else (
            "success" if "JUSTIFIED" in classification or "NO_" in classification else "warning"
        )
        result["tone"] = tone
        return result
