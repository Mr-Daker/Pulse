...
export default function ReasoningPanel({ modelId }) {
  ...
  const run = useCallback(async () => {
    ...
    // Demo fallback — animate pre-computed text
      ...
      <div className="flex-row mb-4" style={{ flexWrap: 'wrap', gap: 12 }}>
        <div style={{ flex: 1, minWidth: 200 }}>
          ...
          <p style={{ fontSize: 13, margin: 0 }}>
            Chain-of-thought bias analysis from the PULSE Reasoner (Groq)
          </p>
        </div>
        <button
        ...
        >
          {state === 'idle' ? 'Run Reasoning' : state === 'running' ? 'Analyzing…' : 'Re-run'}
        </button>
      </div>

      {state === 'idle' ? (
        <div className="rsn-box rsn-empty">
          Click "Run Reasoning" to stream the AI audit chain-of-thought
        </div>
      ) : (
